...
FROM 	products
ORDER BY 	list_price DESC,discount_percent ASC,product_code,product_name;
...
SELECT product_name AS 'The product_name column', list_price AS 'The list_price column', date_added AS "The date_added column"
FROM 	products
WHERE 	(list_price > 500) AND (list_price < 2000) 
ORDER BY 	date_added DESC;

SELECT 	order_id AS 'The order_id column', order_date AS 'The order_date column', ship_date AS "The ship_date column"
FROM 	orders
...
FROM 	customers LEFT OUTER JOIN addresses ON customers.shipping_address_id=addresses.address_id;

SELECT last_name, first_name, order_date, product_name, item_price, discount_amount, quantity
FROM 	customers AS C NATURAL JOIN orders AS O NATURAL JOIN order_items AS OI NATURAL JOIN products AS P
...
SELECT 
    product_name AS 'The product_name column',
    list_price AS 'The list_price column',
    date_added AS 'The date_added column'
FROM
    products
WHERE
    (list_price > 500)
...
SELECT 
    ...
    order_date AS 'The order_date column',
    ship_date AS 'The ship_date column'
FROM
    orders
WHERE
    ship_date IS NULL;
...
CREATE VIEW customer_addressess AS
    SELECT 
        customer_id,
        email_address,
        last_name,
